// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ClientService.cs" company="An Sinh Comapany">
//   Copyright (c) 2021 AnSinhSCADA contributors
// </copyright>
// <summary>
//   Interaction logic for ClientService.cs
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace WpfAppServer
{
    public sealed class DAServer : IDataServer, IAlarmServer, IHDAServer
    {
        ...
        public IDriver AddDriver(short id, string name, string assembly, string className)
        {
            if (reader == null)
            {
                reader = new ClientReader(this, id, name);
            }
            return reader;
        }
        ...
        {
            SystemLog.AddLog(new SystemLog(EventType.Simple, DateTime.Now, App.LogSource, "Alarm response"));
        }
            ...
            while (year <= eyear)
            {
                ...
                while (month <= (year == eyear ? emonth : 12))
                {
                    var result = HDAIOHelper.LoadFromFile((year == syear && month == smonth ? start : new DateTime(year, month, 1)),
                        (year == eyear && month == emonth ? end : new DateTime(year, month, 1).AddMonths(1).AddMilliseconds(-2)), ID);
                    if (result != null)
                    {
        ...
        private IEnumerable<HistoryData> ReadFromFile(DateTime start, DateTime end)
        {
            ...
            while (year <= eyear)
            {
                ...
                {
                    var result = HDAIOHelper.LoadFromFile((year == syear && month == smonth ? start : new DateTime(year, month, 1)),
                        (year == eyear && month == emonth ? end : new DateTime(year, month, 1).AddMonths(1).AddMilliseconds(-2)));
                    if (result != null)
                    {
                        foreach (var data in result)
                            yield return data;
